  ...
  return (
    <footer className="bg-navy-night pt-[52px] pb-[30px] text-white">
      <div className={`${contentShell} grid gap-12`}>
        <Link className="block w-40 sm:w-[200px] lg:w-[220px]" href="/" aria-label={`${brand.name} 홈`}>
          <Image
        ...
        <div className="grid gap-3 border-t border-white/25 pt-[18px] text-[11px] text-white/60 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <a className="w-fit hover:text-white" href={`mailto:${brand.contactEmail}`}>{brand.contactEmail}</a>
          <Link className="w-fit hover:text-white" href="/privacy">개인정보 처리 안내</Link>
          <p className="m-0 lg:text-right">© {new Date().getFullYear()} {brand.name} · Seoul</p>
        </div>
      </div>
    </footer>
  );
