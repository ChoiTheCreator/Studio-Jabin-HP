      ...
      <div className={contentShell}>
        <Reveal className="grid gap-8 border-t border-navy-line pt-5 lg:grid-cols-[minmax(240px,0.8fr)_minmax(0,2.2fr)] lg:gap-16 lg:pt-6">
          <div className="flex items-start justify-between gap-5 lg:block">
            ...
            <dl className="m-0 grid grid-cols-2 gap-x-5 text-right lg:mt-20 lg:max-w-[220px] lg:text-left">
              <div>
              ...
              <div>
                ...
                <dd className="mt-1 text-[26px] leading-none font-bold">
          ...
          <div>
            ...
            <p className="mt-7 mb-0 max-w-[620px] text-[16px] leading-[1.68] text-navy-muted [word-break:keep-all] sm:text-[17px]">
              기획과 기술의 책임자가 처음부터 직접 참여합니다. 빠른 의사결정과 명확한
              책임 범위로 아이디어를 실제 운영 가능한 서비스까지 연결합니다.
            </p>
          </div>
        </Reveal>
        ...
        <div className="mt-14 border-t border-navy-line sm:mt-24 lg:mt-24">
          {team.members.map((member, memberIndex) => (
            <Reveal key={member.number} delay={memberIndex * 100}>
              <article className="group relative grid gap-7 overflow-hidden border-b border-navy-line py-7 sm:grid-cols-[88px_minmax(180px,0.8fr)_minmax(0,1.2fr)] sm:gap-6 sm:py-9 lg:min-h-[284px] lg:grid-cols-[88px_minmax(220px,0.85fr)_minmax(300px,1.2fr)_minmax(180px,0.65fr)] lg:items-start lg:gap-9 lg:py-9">
                <div className="flex items-center justify-between sm:block">
                  <span className="text-[12px] font-bold text-navy-primary">{member.number}</span>
                  <span
                    className="grid size-14 place-items-center bg-navy-deep text-[15px] font-bold text-white transition-colors duration-300 group-hover:bg-navy-primary sm:mt-[74px] sm:size-16 lg:size-[68px]"
                    aria-hidden="true"
                  >
                    {member.initials}
                  </span>
                ...
                <div>
                  <p className="m-0 max-w-[570px] text-[17px] leading-[1.65] [word-break:keep-all] sm:text-[18px]">
                    {member.responsibility}
                  </p>
                  <p className="mt-7 mb-0 max-w-[520px] text-[12px] leading-[1.7] font-medium text-navy-muted">
