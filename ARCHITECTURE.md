...
현재 필수 환경 변수는 없다. 공개 도메인처럼 배포 환경과 무관하게 고정된 값은 환경 변수 대신 `src/config`의 상수로 관리한다. `NEXT_PUBLIC_` 값은 빌드 시점에 번들에 새겨지므로 런타임 주입으로는 바꿀 수 없다는 점을 전제로 판단한다.

선택 값은 다음 하나이며, 설정하지 않아도 서비스는 정상 동작한다.

| 이름                  | 용도                                 | 없을 때 동작                                             |
| --------------------- | ------------------------------------ | -------------------------------------------------------- |
| `INQUIRY_WEBHOOK_URL` | 접수된 문의를 전달할 외부 엔드포인트 | 전송하지 않고 민감 정보를 가린 요약만 서버 로그에 남긴다 |
